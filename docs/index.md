...
| bioschemas:codeRepository | Link to the repository where the un-compiled, human readable code and related code is located (SVN, github, CodePlex).  |
| bioschemas:input | Specification of a consumed input.  |
| bioschemas:output | Specification of a produced output.  |
| bioschemas:programmingLanguage | The computer programming language.  |
| dct:conformsTo | Used to state the Bioschemas profile that the markup relates to. The versioned URL of the profile must be used. Note that we use a CURIE in the table here but the full URL for Dublin Core terms must be used in the markup (http://purl.org/dc/terms/conformsTo), see example.  |
